...
def utility_score(title, preferred_genre, mood_genres, collab_scores, ref_genres=None):
    ...
    mood_score   = min(len(genres & set(mood_genres)) / max(len(mood_genres), 1), 1.0) if mood_genres else 0.0
    rating_score = (avg_r - 0.5) / 4.5
    collab_score = min(collab_scores.get(title, 0) / 25.0, 1.0)

    # Boost score significantly if it shares genres with the reference movie
    if ref_genres:
        ref_score = min(len(genres & set(ref_genres)) / max(len(ref_genres), 1), 1.0)
        return round(genre_score*0.15 + mood_score*0.15 + ref_score*0.40 + rating_score*0.20 + collab_score*0.10, 4)
...
def smart_recommend(user_id, mood, mood_genres, preferred_genre, ref_movie, profile, n=15):
    ...
    # Extract ref_genres if a ref_movie was provided
    ref_genres = []
    if ref_movie:
        ref_row = df[df['title'] == ref_movie]
        if not ref_row.empty:
            ref_genres = ref_row.iloc[0]['genres'].split('|')

    # Score and rank
    scored = []
    for title, source in pool_filtered.items():
        score      = utility_score(title, preferred_genre, mood_genres, collab_scores, ref_genres)
